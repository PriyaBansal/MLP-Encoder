...
//	             (ChannelParams (*)[ctx->avctx->channels]) &channel_params[ctx->restart_intervals - 1][ctx->seq_offset[ctx->restart_intervals - 1]];
        ...
        for (index = 0; index < ctx->seq_size[ctx->restart_intervals-1]; index++) {
            memcpy(&ctx->major_decoding_params[index][substr], &decoding_params[(ctx->restart_intervals -1) * ctx->sequence_size                                  * ctx->num_substreams + (ctx->seq_offset[ctx->restart_intervals -1])* ctx->num_substreams][ctx->num_substreams*index + substr], sizeof(DecodingParams));
            for (channel = 0; channel < ctx->avctx->channels; channel++) {
